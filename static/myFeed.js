...
const createPost = () => {
  ...
  xhr.open('POST', 'http://localhost:5000/api/post', true)
...
const createPostDiv = (post, element) => {
  ...
  post.addEventListener('click', showPost(post.id))
  return post
}
...
const loadPosts = () => {
  let xhr = new XMLHttpRequest()
  xhr.open('GET', 'http://localhost:5000/api/myFeed', true)
  xhr.send()
  xhr.onreadystatechange = () => {
    if (xhr.readyState === 4 && xhr.status === 200) postContainer = listPosts(JSON.parse(xhr.responseText), postContainer)
  }
}
